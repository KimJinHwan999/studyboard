...
public class BoardService {
	...
	public BoardDTO findAllByBoardNum(Long board_num) {
		return boardMapper.findAllByBoardNum(board_num);
	}
	
	/* 하이차트 데이터 1 - 게시판 별 업로드 된 글의 수 */
	/*
	 * public List<Long> selectBoardNum_(){ return boardMapper.selectBoardNum_(); }
	 * 
	 * public List<HchartBoardDTO> selectHchartBoardNum(List<Long> boardNumList) {
	 * for(int i = 0; i < boardNumList.size(); i++) {
	 * System.out.println(boardNumList.get(i)); } return
	 * boardMapper.selectHchartBoardNum(boardNumList); }
	 * 
	 * public List<Map<Long, Long>> selectBoardNum(){ return
	 * boardMapper.selectBoardNum(); }
	 */
	...
	public Long hchartBoardCnt(Long boardNum){
		return boardMapper.hchartBoardCnt(boardNum);
	}
